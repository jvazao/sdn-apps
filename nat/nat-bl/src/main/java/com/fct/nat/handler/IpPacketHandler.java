...
public class IpPacketHandler {
	...
	public void dns(MessageContext mc, Ethernet ethData, Ip ipData, Udp udpData, BigPortNumber inPort) {
		...
		data.save(new Date(), EthernetType.IPv4, ethData.srcAddr(),
				ipData.srcAddr(), ipData.dstAddr(), IpProtocol.UDP,
				udpData.srcPort(), udpData.dstPort(), (long) port.toInt());

		// REVERSE FLOW
		OfmMutableFlowMod dstSrcFlow = 
				(OfmMutableFlowMod) MessageFactory.create(PV, MessageType.FLOW_MOD, FlowModCommand.ADD);
	...
	public void icmp(boolean toInternet, MessageContext mc, Ethernet ethData, Ip ipData, MacAddress targetMacAddr, BigPortNumber inPort) {
		...
		// Save the direct flow information
		data.save(new Date(), EthernetType.IPv4, ethData.srcAddr(),
				ipData.srcAddr(), ipData.dstAddr(), IpProtocol.ICMP, null,
				null, (long) port.toInt());
		...
		// Save the reverse flow information
		data.save(new Date(), EthernetType.IPv4, ethData.srcAddr(),
				ipData.dstAddr(), ipData.srcAddr(), IpProtocol.ICMP, null,
				null, (long) port.toInt());
	...
	public void tcp(boolean toInternet, MessageContext mc, Ethernet ethData, Ip ipData, Tcp tcpData, MacAddress targetMacAddr, BigPortNumber inPort) {
		...
		// Save the direct flow information
		data.save(new Date(), EthernetType.IPv4, ethData.srcAddr(),
				ipData.srcAddr(), ipData.dstAddr(), IpProtocol.TCP,
				tcpData.srcPort(), tcpData.dstPort(), (long) port.toInt());
		...
		// Save the reverse flow information
		data.save(new Date(), EthernetType.IPv4, ethData.dstAddr(),
				ipData.dstAddr(), ipData.srcAddr(), IpProtocol.TCP,
				tcpData.dstPort(), tcpData.srcPort(), (long) port.toInt());
	...
	public void udp(boolean toInternet, MessageContext mc, Ethernet ethData, Ip ipData, Udp udpData, MacAddress targetMacAddr, BigPortNumber inPort) {
		...
		data.save(new Date(), EthernetType.IPv4, ethData.dstAddr(),
				ipData.srcAddr(), ipData.dstAddr(), IpProtocol.UDP,
				udpData.dstPort(), udpData.srcPort(), (long) port.toInt());
	...
	public void icmpReply(MessageContext mc, Ethernet ethData, Ip ipData, Icmp icmpRequest, BigPortNumber inPort) {
		...
		Ethernet ethPacket = new Ethernet.Builder()
