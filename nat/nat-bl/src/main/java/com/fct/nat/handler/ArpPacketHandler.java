...
public class ArpPacketHandler {
	...
	/**
	 * Returns the MacAddress of the corresponding IpAddress in the Layer2
	 * connected network, or null if none found.
	 * */
	 ...
	 * constructs the ARP Table of that network. This is needed so that the SVI
	 * can send traffic to hosts on that Network.
	 * */
	public void handle(Arp arpRequest) {
		LOG.info("NAT: ArpPacketHandler: handle()");

		if (!arpTable.containsKey(arpRequest.senderIpAddr()) && !arpTable.containsValue(arpRequest.senderMacAddr())) {
	...
	public void reply(Arp arpRequest, BigPortNumber inPort) {
		...
		Arp arpPacket = new Arp.Builder().opCode(OpCode.REPLY)
				...
				.targetMacAddr(arpRequest.senderMacAddr()).build();
	...
	/**
	 * Constructs ARP REQ on behalf of the SVI to a host on the connected
	 * network.
	 * */
